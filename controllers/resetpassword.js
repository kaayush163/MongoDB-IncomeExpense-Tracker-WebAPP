...
const forgotpassword = async (req, res) => {  
    try {
        ...
        const user = await User.find({email:email});  /// user table se pura wo specific email wala row lelega
        if(user){     
            ...
            await Forgotpassword.create({uuid:id,active: true,userId:user[0]._id })  ///// createForgotpassword is a sequelize method function
                .catch(err => {
                    throw new Error(err)
                })
...
const resetpassword = async(req, res) => {
    try{
        ...
        console.log(err)
        res.status(500).json({message: 'Internal Server Error 500, Link not working in resetpassword', err: err})
    }
}
...
const updatepassword = async(req, res) => {
    ...
    try {
        ...
        if(row){
            ...
            const forgot=Forgotpassword.findOneAndUpdate({uuid: resetpasswordid}, {$set: {active: false}})

            console.log(forgot);
            const salt = await bcrypt.genSalt(10)
            const hashedPass = await bcrypt.hash(newpassword, salt)

            const updatepass= await User.findOneAndUpdate({_id: objectuserId}, {$set: {password: hashedPass}})
        ...
        return res.status(403).json({ error:error,message:"some problem occur in update", success: false } )
    }
